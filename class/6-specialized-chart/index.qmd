...
- **Context**: Collected from the Montesinho Natural Park in Portugal, this dataset captures weather conditions and fire activity over 200+ days. The task was to predict the burned area of forest fires based on meteorological and fire weather indices.

- **Variables**:
  - `month`, `day`: Temporal context
  - `temp`, `RH`, `wind`, `rain`: Daily weather
  - `FFMC`, `DMC`, `DC`, `ISI`: Fire weather indices
  - `area`: Outcome of interest, burned area in hectares
...
> PCP does not show the classic Cartesian coordinate plane, but each numerical variable is given its own axis. All axes are placed **parallel**, **vertical**, and **equally spaced**.
